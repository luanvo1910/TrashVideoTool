...
def process_video(audio_url, video_url1, video_url2, video_speed1, video_speed2, 
    ...
    try:
        ...
        for i in range(actual_num_parts):
            ...
            input_map = {'video-placeholder-1': 0, 'video-placeholder-2': 1, 'thumbnail-placeholder': 2}
            image_index = 3
            for item in layout:
...
if __name__ == "__main__":
    # In ra ngay từ đầu để đảm bảo script đã chạy
    try:
        print("STATUS: Python script đã khởi động...", flush=True)
    except:
        # Nếu không in được, có thể là encoding issue
        import sys
    ...
    try:
        ...
        print(f"STATUS: Đã nhận arguments, resources-path: {args.resources_path}", flush=True)
        
        # Tự động cài đặt yt-dlp nếu chưa có
        print("STATUS: Kiểm tra yt-dlp...", flush=True)
        if not ensure_yt_dlp():
            print("PYTHON_ERROR: Không thể cài đặt yt-dlp", flush=True)
            sys.exit(1)
        
        # Import yt_dlp (đã được cài đặt hoặc đã có sẵn)
        print("STATUS: Import yt-dlp...", flush=True)
        import yt_dlp
        print("STATUS: yt-dlp đã được import thành công", flush=True)
        
        print("STATUS: Bắt đầu xử lý video...", flush=True)
        process_video(
            args.audio_url, args.video_url1, args.video_url2, 
            args.video_speed1, args.video_speed2, args.parts, args.save_path, 
            args.part_duration, args.layout_file, args.encoder, 
            args.resources_path, args.user_data_path
        )
        print("STATUS: Hoàn tất xử lý video", flush=True)
        sys.exit(0)  # Thành công
    except SystemExit as e:
        # Re-raise SystemExit để giữ nguyên exit code
        raise
    except Exception as e:
        import traceback
        error_msg = str(e)
